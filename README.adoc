...
NOTE: You will need to add `spring-boot-starter-restclient` as a test dependency for the integration tests to work.
...
. Add `spring-boot-starter-restclient` as a test dependency to your build file for the integration tests.
...
The response is written using one of Spring's message converters. Because Jackson 2 is in the classpath,
...
NOTE: The test uses Spring's `RestClient` for making HTTP requests. In Spring Boot 4.x, `RestClient.Builder` is auto-configured and can be autowired in tests when `spring-boot-starter-restclient` is present on the test classpath.
